...
class Game:
    ...
    def heuristicSimple(self):
        ...
        print("diagonal2")
        for x in range(-self.N + 1, self.N):
            xCount = self.symbolCount(
                np.flipud(self.current_state).diagonal(x), "X")
            yCount = self.symbolCount(
                np.flipud(self.current_state).diagonal(x), "O")
            total += self.valueAttributor(xCount, yCount)
            print()
